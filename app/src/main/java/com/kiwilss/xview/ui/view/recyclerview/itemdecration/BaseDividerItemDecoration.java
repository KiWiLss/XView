...
public abstract class BaseDividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public BaseDividerItemDecoration(Context context, float lineWidthDp, @ColorInt int colorRGB) {
...
//        this.lineWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, lineWidthDp, context.getResources().getDisplayMetrics());
    ...
    public BaseDividerItemDecoration(Context context, int lineWidthDp, @ColorInt int colorRGB) {
        this.colorRGB = colorRGB;
        this.lineWidth= lineWidthDp;
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(colorRGB);
        mPaint.setStyle(Paint.Style.FILL);
    }
